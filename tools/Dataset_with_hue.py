...
#         return patch_path, augmented_image, patch_label, idx
...
class Dataset(Dataset):
    def __init__(self, dataset_dir, model_mode, channel_means, channel_stds):
        ...
        if model_mode == 'train':
            ...
            # Define ColorJitter for tissue patches only
            self.color_jitter = transforms.ColorJitter(brightness=0, contrast=0, saturation=0, hue=0.4)
    ...
    def __getitem__(self, idx):
        ...
        # Conditionally apply ColorJitter for tissue patches only
        if self.model_mode == 'train' and patch_label == 1:
            augmented_image = self.color_jitter(augmented_image)
...
class Reg_Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return patch_path, augmented_image, tissue_ratio, idx
